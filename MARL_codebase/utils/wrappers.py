...
import warnings
from gymnasium import Wrapper, ObservationWrapper, RewardWrapper, spaces
# from gymnasium.wrappers import monitoring, time_limit
...
class RecordEpisodeStatistics(Wrapper):
    ...
    def reset(self, **kwargs):
        observation, infos = super().reset(**kwargs)
...
class SquashDones(Wrapper):
    r"""Wrapper that squashes multiple dones to a single one using all(dones)"""

    def step(self, action):
        observation, reward, done, truncated, info = self.env.step(action)
        return observation, reward, all(done), truncated, info


class ObserveID(ObservationWrapper):
    def __init__(self, env):
        super().__init__(env)
...
class GlobalizeReward(RewardWrapper):
    def reward(self, reward):
        return self.n_agents * [sum(reward)]


class CooperativeReward(RewardWrapper):
    def reward(self, reward):
        return [sum(reward)]


class StandardizeReward(RewardWrapper):
...
# class TimeLimit(time_limit):
#     def __init__(self, env, max_episode_steps=None):
#         super().__init__(env)
#         if max_episode_steps is None and self.env.spec is not None:
#             print("max_episode_steps é None")
#             print(f"self.env.spec é {self.env.spec}")
...
class ClearInfo(Wrapper):
    def step(self, action):
        observation, reward, done, truncated, info = self.env.step(action)
        return observation, reward, done, {}


class SMACCompatible(Wrapper):
    def __init__(self, env):
        super().__init__(env)
